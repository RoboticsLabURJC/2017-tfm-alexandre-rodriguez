...
\caption{dl-objecttracker module architecture}
\label{fig:general}
\end{center}
\end{figure}
...
When the detection is available, the Tracker thread starts the tracking of the detected objects in the buffer of frames. The last frame in the buffer is used to feed again the Network allowing for a synchronism between the detections and the tracking in the frames in an online mode. This temporal process can be observed more clearly in the Figure \ref{fig:buffer}.
\begin{figure}[H]
\begin{center}
...
\end{center}
\end{figure}
It can be seen that given a complete buffer in Camera of size \textit{N}, the first frame is assigned to the Network and, when this frame is processed, a buffer of size \textit{N-2} is assigned to the Tracker to be processed. The last frame of the given buffer was passed to the Network again. This iterative mechanism continues in time ensuring that no frames are lost. The application has always a delay in frames of 1 buffer (\textit{simulated real-time}).
...
\section{How to launch the dl-objecttracker}
All the modules that form the component have been presented, the application can be put into operation. After modifying the configuration accordingly the application can be runned following this instructions:
\begin{enumerate}
    \item \textbf{Open a terminal}: open a terminal where the \textit{objecttracker.py} is located.
    \item \textbf{Launch the dl-objecttracker}: in this terminal type\\ \texttt{python2 objecttracker.py objecttracker.yml on}
\end{enumerate}
The GUI can be turned on or off with the argument that comes after the configuration file (\textit{on/off}).